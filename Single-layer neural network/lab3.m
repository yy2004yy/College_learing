...
num_correct = sum(all(labels ~= [0, 0], 2) & all(prediction == labels, 2));

num_total = sum(all(labels ~= [0, 0], 2));

error_rate = 1 - num_correct / num_total;

% Print the original size and predicted label for each data point
for i = 1:size(test_data, 1)
    if all(test_data(i,end-1:end) ~= [0, 0])
...
function [train_data, test_data] = load_data()
    ...
    idx = trainLabels == 5;
    train_labels(idx, :) = repmat([-1, -1], sum(idx), 1);
    idx = trainLabels == 0 | trainLabels == 3| trainLabels == 4| trainLabels == 6| trainLabels == 8| trainLabels == 9;
    train_labels(idx, :) = repmat([0, 0], sum(idx), 1);
    ...
    idx = testLabels == 5;
    test_labels(idx, :) = repmat([-1, -1], sum(idx), 1);
    idx = testLabels == 0 | testLabels == 3 | testLabels == 4 | testLabels == 6 | testLabels == 8 | testLabels == 9;
    test_labels(idx, :) = repmat([0, 0], sum(idx), 1);
...
function prediction = predict(weights1, bias1, weights2, bias2, data)
    prediction = zeros(size(data,1), 2);
    ...
    for i = 1:size(data, 1)
        ...
        if output1 == 1
            ...
            if output2 == 1
                prediction(i, 2) = 1; % Label (0, 1, 4, 5 ,6)
            elseif output2 == -1
                prediction(i, 2) = -1; 
            end
